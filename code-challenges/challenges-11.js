...
function fullName(myArray) {
  return myArray.map((a) => `${a.firstName} ${a.lastName}`);
}
...
function gradesAvg(myArray) {
  const average = (myArray) => myArray.reduce((a, b) => a + b, 0) / myArray.length;
  myArray.map((avg) => (avg["avg"] = average(avg.gradsList)));
  return myArray;
}
...
function studentsResult(myArray) {
  myArray.map((obj) => (obj.result = obj.avg >= 50 ? "Passed" : "Failed"));
  return myArray;
}
