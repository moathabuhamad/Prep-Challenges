...
const filterLinks = (str) => {
  const UrlStart = "http://";
  const UrlEnd = str.includes(".com")
    ? ".com"
    : str.includes(".org")
    ? ".org"
    : ".net";
  const indexOfStart = str.indexOf(UrlStart) + UrlStart.length;
  const indexOfEnd = str.indexOf(UrlEnd) + UrlEnd.length;
...
const isPalindrome = (str) => {
  let pali = /[^0-9a-z]/gi;
  let result = str.replace(pali, "").toLowerCase();
